...
public class DeviceListActivity extends Activity {//设备列表视图
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		// Setup the window
		setContentView(R.layout.device_list);//设置视图组件
		...
		mPairedDevicesAdapter = new MyExpandableListAdapter(getApplicationContext(),
				...
				new int[] { R.id.img_deviceType_1, R.id.txt_deviceName_1, R.id.txt_deviceAddress_1,R.id.img_onright_1 });//已配对设备列表
	...
	private void discoveryable() {//使蓝牙设备可见
		if (mBtAdapter.getScanMode() !=
				BluetoothAdapter.SCAN_MODE_CONNECTABLE_DISCOVERABLE) {//SCAN_MODE_CONNECTABLE_DISCOVERABLE表示设备既可以被远程蓝牙设备发现，也可以被其连接
			Intent discoverableIntent = new Intent(BluetoothAdapter.ACTION_REQUEST_DISCOVERABLE);//请求蓝牙设备可被发现
			discoverableIntent.putExtra(BluetoothAdapter.EXTRA_DISCOVERABLE_DURATION, 120);//设置蓝牙设备可被发现的持续时间
	...
	private final BroadcastReceiver mReceiver = new BroadcastReceiver() {//广播接收器
		...
		public void onReceive(Context context, Intent intent) {
			...
			} else if (BluetoothAdapter.ACTION_DISCOVERY_FINISHED.equals(action)) {//如果搜索完成
